...
function isPalindrome(str) {
  const strWithoutSpaces = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
  const middleIndex = Math.floor(strWithoutSpaces.length / 2);
  const firstHalfStr = strWithoutSpaces.slice(0, middleIndex);
  const secondHalfStr = strWithoutSpaces.slice(
    strWithoutSpaces.length % 2 === 0 ? middleIndex : middleIndex + 1
  );
...
function findLongestWord(str) {
  const arrSubStr = str.split(' ');
  let longestWord = arrSubStr[0];
  for (let i = 0; i < arrSubStr.length; i += 1) {
    if (arrSubStr[i].length > longestWord.length) longestWord = arrSubStr[i];
  }
  return longestWord;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arrChars.length; i += 1) {
    if (arrChars[i] === arrChars[i].toUpperCase()) {
      arrChars[i] = arrChars[i].toLowerCase();
    } else {
      arrChars[i] = arrChars[i].toUpperCase();
    }
  }
...
function extractNameFromTemplate(str) {
  const arrWords = str.split(' ');
  arrWords[2] = arrWords[2].slice(0, -1);
  return [arrWords[1], arrWords[2]].join(' ');
}
...
function encodeToRot13(str) {
  const firstHalfAlphabet = 'abcdefghijklmABCDEFGHIJKLM';
  const secondHalfAlphabet = 'nopqrstuvwxyzNOPQRSTUVWXYZ';
  const arrChars = str.split('');
  for (let i = 0; i < arrChars.length; i += 1) {
    if (firstHalfAlphabet.includes(arrChars[i])) {
      arrChars[i] = secondHalfAlphabet[firstHalfAlphabet.indexOf(arrChars[i])];
    } else if (secondHalfAlphabet.includes(arrChars[i])) {
      arrChars[i] = firstHalfAlphabet[secondHalfAlphabet.indexOf(arrChars[i])];
    }
  }
  return arrChars.join('');
}
